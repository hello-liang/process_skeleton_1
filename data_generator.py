...
class DataGenerator():
    ...
    def get_pose_data_v2(self, body, validation, rotation_matrix=None):
        ...
        body = body[np.all(~np.all(body==0, axis=2), axis=1)]         # here seems have some problem??
...
# =============================================================================
        ...
        if not validation and self.temporal_scale is not False:# pass
        ...
        if len(self.skip_frames) > 0:
            ...
            if len(body[sk_init::sk]) >= 2: body = body[sk_init::sk] # 141 >2 so is 141


        # 4. Modify movement speed
        if self.max_seq_len > 0: #is 0 so pass
        ...
        # 5. Scale by torso
        if self.scale_by_torso: body = self.scale_skel(body)       # here  normalization
